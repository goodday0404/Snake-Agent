...
class ActorCritic:
    ...
    def load_model(self, fn):
        ...
        # opt1 = tf.keras.optimizers.Adam(lr = self.learning_rate)
        # self.model.compile(optimizer = opt1, loss = self.custom_loss_function)

        self.target_model = tf.keras.models.load_model(fn, custom_objects = {'custom_loss_function' : self.custom_loss_function})
        # opt2 = tf.keras.optimizers.Adam(lr = self.learning_rate)
        # self.target_model.compile(optimizer = opt2, loss = self.custom_loss_function)
    ...
    def act(self, cur_state):
        ...
        """
        # need to change this to linear decay ??
    ...
    def replay(self, batch_size=32):
        ...
        #     target[a] = predicted[index]
        #     index += 1

        #samples = self.sample_memories(batch_size)
        # states = []
        # targets = []
    ...
    def sample_memories(self, batch_size=32):
        ...
        # mem1_batch_size = int(np.round(self.eta * batch_size * p1))
        # game_mem1_batch_size = batch_size1 - mem1_batch_size
        # mem2_batch_size = int(batch_size2 * p2)
        ...
        # mem1_batch_size = int(np.round(SAMPLING_PROP * batch_size))
        # mem2_batch_size = batch_size - mem1_batch_size
        #result = []
